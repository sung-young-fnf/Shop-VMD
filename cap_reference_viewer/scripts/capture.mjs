...
await page.goto('http://127.0.0.1:5187');await page.waitForSelector('body[data-ready="true"]');await page.waitForTimeout(800);
for(const view of ['three','front','side','rear','inside','detail']){await page.locator(`[data-view="${view}"]`).click();await page.waitForTimeout(500);await page.screenshot({path:`evidence/${view}.png`,fullPage:true});}
...
const refs=[];for(const ref of ['0','5','4','6','7','8']){await page.locator(`[data-ref="${ref}"]`).click();await page.waitForTimeout(150);refs.push(await page.locator('#reference').evaluate(img=>({src:img.src,loaded:img.complete&&img.naturalWidth>0})));}
