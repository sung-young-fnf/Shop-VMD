...
function meshSnapshot(group, selected, exact = false) {
  ...
  group.traverse(object => {
    ...
    const attributes = ['capSourcePosition','capFrontUv','capRearUv','capSideUv','capFabricUv','softPhotoWeight','softSourcePosition'].map(name=>[name,geometry.getAttribute(name)]).filter(([,item])=>item);
    const custom=[];
    for(let vertex=0;vertex<position.count;vertex++)for(const [name,item] of attributes){
      custom.push(name);for(let axis=0;axis<item.itemSize;axis++)custom.push(item.array[vertex*item.itemSize+axis]);
    }
    ...
    for(const value of custom)source.push(value);
    if (exact) {
      for(const value of custom)ordered.source.push(value);
      correspondence.set(key, ordered);
    }
  });
  const result = { vertices: points.length, points: hash(points.sort()), source: hash(source) };
...
try {
  ...
  for (const [fixture, batch] of fixtures) {
    ...
    for (const [index, cap] of caps.entries()) {
      ...
      if (id.startsWith('ca-')) {
        check(bounds.min.z >= -.18 && bounds.max.z <= .185, `${id}/${index}: cap exceeds cabinet shelf depth`);
        check(bounds.max.y < support + .333, `${id}/${index}: headwear intersects next shelf`);
      }
  ...
  if (stage !== 'red') {
    const baseline = JSON.parse(await readFile('evidence/cap-store-integration-20260908/red.json', 'utf8'));
    check(hash(clothing) === hash(baseline.clothing), 'Clothing world geometry differs from frozen pre-fix actual fixture baseline');
    check(new Set(records.filter(row=>row.fixture.startsWith('ca-')).slice(0,76).map(row=>row.sku)).size===76,'First wall cycle must contain all 76 distinct admitted SKUs');
  }
