...
| 항목 | 값 |
|---|---|
| 형태 ID | `_NEW_미채번/M25SLPS02_스키퍼플립플랍` |
| 라스트 (LAST NO.) | `NEW` |
| 아웃솔 (OUTSOLE NO.) | `NEW` |
| style_code | `M25SLPS02` (1개) |
...
|---|---|---|---|---:|---|---|---|---|
| `3ALPS0253` | 스키퍼 플립플랍 | 25SS / 1st | 25S | 4 | 07BLL, 43PKM, 50BKS, 50WHS |  | NEW | NEW(=MLBM-2514 2D 도면 공유) |

> **색만 다른 파생은 이 표로 처리한다.** 형상 재작업 없이 머티리얼만 교체. 도면 1장은 헤더 컬러 1개 기준으로 그려져 있고, 나머지 컬러는 KG 컬러 코드로만 확인된다.

### 귀속 사유 (도면 표기와 형태 ID 가 다른 스타일)
- `3ALPS0253` — 라스트 NEW. 아웃솔은 "SAME AS MLBM-2514-2D BLUEPRINT" → 스키퍼 슬라이드 솔 공유, 갑피(통 스트랩)는 별도 형태
...
| PART_CD | 치수 콜아웃 | 뷰 |
|---|---|---|
| `3ALPS0253` | 11mm;Raised 1.0mm;2.0mm;R7.0;R2.0;Raised 0.3mm;Recessed 0.7mm | 측면;저면;풋베드로고 |

치수는 240mm 기준 스팟 치수(로고 높이·웨빙 폭·펀칭 지름·두께 등)다. **부위 전체 치수·사이즈별 그레이딩 표는 어느 소스에도 없다.**

### 파트 구성 · 자재 (도면 콜아웃 기준 — BOM 데이터 없음, 여기가 원천)
<details><summary><code>3ALPS0253</code> 스키퍼 플립플랍 — 25SS 1st · AW · JAEGON YOO · 2024.06.26</summary>
...
- `3ALPS0253`: 이지슬라이드 도면 참고 지시문(1.라스트 2.실루엣 3.두께·R값). *OUTSOLE SAME AS MLBM-2514-2D BLUEPRINT* → 스키퍼 슬라이드(MLBM-2514)와 아웃솔 공유. 풋베드 로고 EVA SLIDE 동일. 저면 쉐브론 트레드 도면 있음

## 컬러웨이 적용 규칙
| PART_CD | 도면 컬러칩 (팬톤/코드) | KG 컬러 코드 (전개) |
|---|---|---|
| `3ALPS0253` | L.BLUE(SAMPLE COLOR); W3 WHITE(WHS) | 07BLL, 43PKM, 50BKS, 50WHS |
...
- 도면에 외부/실물 참고 컷이 붙어 있는 스타일: `3ALPS0253` — 디자인 의도 파악용이며 치수 근거는 아니다.
- 도면에는 자재 품번·협력사·담당자 실명·원가 관련 정보가 있다. **외부 공유·커밋 전 취급 확인 필수.**

## 출처
- `3ALPS0253` 도면: https://s3.ap-northeast-2.amazonaws.com/static-plm.fnf.co.kr/style/3ALPS0253.jpg · 제품컷: https://static-resource-mall.fnf.co.kr/mlb-korea/images/goods/ec/M25S3ALPS025350WHS/thnail/592D077D1082404CA1D5993674C4EC58.png/dims/resize/200x200
