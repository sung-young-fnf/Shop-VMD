...
try {
 for(let run=0;run<3;run++) {await page.goto('http://127.0.0.1:4174');await page.waitForFunction(()=>window.__MLB_DEBUG__?.sceneReady&&window.__MLB_DEBUG__.cameraSettled);report.startup.push(await page.evaluate(()=>({startup:window.__MLB_DEBUG__.startup,stats:window.__MLB_DEBUG__.stats})));}
